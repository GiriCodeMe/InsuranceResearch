...
Use this skill when:
- The user describes a new feature to build ("I want to add...", "Build me a...")
- Starting any non-trivial feature — always spec first, before implementation
- An existing spec needs to be revised with new requirements

Do NOT use if a spec already exists — use `/speckit.clarify` or `/speckit.plan` instead.

## Output

- New feature branch checked out (`{number}-{short-name}`)
- `spec.md` written to the feature directory
- `checklists/requirements.md` — initial quality checklist with pass/fail status

## Pre-flight Checklist

- [ ] `.specify/` directory exists in project root
- [ ] User has provided a feature description (non-empty)
- [ ] No existing spec for this feature (if updating, confirm intent)
